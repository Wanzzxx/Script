...
-- Auto Rejoin If Stuck
Options.AutoRejoinIfStuck = Tabs.Misc:AddToggle("AutoRejoinIfStuck", {
    Title = "Auto Rejoin If Stuck",
    Default = false,
    Description = "Rejoins if no fish caught for 2 minutes"
})

Options.AutoRejoinIfStuck:OnChanged(function()
    if Options.AutoRejoinIfStuck.Value then
        Fluent:Notify({ Title = "Auto Rejoin If Stuck", Content = "Enabled - Monitoring for 2 minutes", Duration = 3 })
        task.spawn(function()
            local player = game:GetService("Players").LocalPlayer
            ...
            display.DescendantAdded:Connect(function(descendant)
                if Options.AutoRejoinIfStuck.Value and descendant:IsA("TextLabel") and descendant.Name == "ItemName" then
                    local itemName = descendant.Text or ""
            ...
            -- Check every 10 seconds if stuck
            while Options.AutoRejoinIfStuck.Value do
                task.wait(10)
                if Options.AutoRejoinIfStuck.Value then
                    local timeSinceLastItem = tick() - lastItemTime
                    if timeSinceLastItem >= 120 then -- 2 minutes
                        Fluent:Notify({ 
                            Title = "Auto Rejoin", 
                            Content = "No fish for 2 minutes - Rejoining...", 
                            Duration = 5 
                        })
                        task.wait(1)
        ...
        Fluent:Notify({ Title = "Auto Rejoin If Stuck", Content = "Disabled", Duration = 3 })
    end
end)
